...
impl VersionMeta {
    ...
    ///
    /// WORKAROUND NOTE: This function replaces `Command::output()` with manual
    /// `spawn() + read_to_end + wait()` because `std::sys::process::output()`
    /// panics on Windows when the user path contains non-ASCII characters
    /// (e.g., Chinese username). The panic occurs at
    /// `library/std/src/sys/process/mod.rs:65` — `res.unwrap()` on
    /// `Err(io::Error { code: 0, kind: Uncategorized })`, where code 0 is
    /// ERROR_SUCCESS but is incorrectly wrapped as Err by ReadFile/GetLastError.
    pub fn for_command(mut cmd: Command) -> Result<VersionMeta> {
        ...
        let mut child = cmd.spawn().map_err(Error::CouldNotExecuteCommand)?;
        let mut stdout = Vec::new();
        let mut stderr = Vec::new();

        // Read stdout and stderr manually. We deliberately IGNORE read_to_end
        // errors here: on Windows with non-ASCII user paths, read_to_end can
        // return Err(code: 0, "操作成功完成") while still having populated the
        // buffer with valid data. Discarding the error lets us proceed with
        ...
        let status = child.wait().map_err(Error::CouldNotExecuteCommand)?;
        ...
        version_meta_for(str::from_utf8(&stdout)?)
    }
}
